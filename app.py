...
def ui_controls():
    with st.sidebar:
        ...
        top_default = int(st.session_state.get("top_offset_px", 64))
        top_px = st.number_input("Sticky tabs offset (px)", min_value=0, max_value=200, value=top_default, step=2, key="top_offset_px")
        st.caption("If the top tabs are hidden by the GitHub/share bar, increase this.")
    return int(top_px)
...
def render_css():
    ...
    /* --- QS palette overrides for sliders & controls (no red) --- */
    .stSlider [data-baseweb="slider"]>div>div,
    .stSlider [data-baseweb="slider"]>div>div>div { background: var(--qs-blue500)!important; }
    .stSlider [role="slider"] { background: var(--qs-blue500)!important; border-color: var(--qs-blue500)!important; }
    .stSlider [data-testid="stTickBar"] div { background: var(--qs-blue500)!important; }
    .stSlider [data-baseweb="slider"]>div>div>div>div { background: var(--qs-blue500)!important; }
    .stRadio [role="radio"][aria-checked="true"] { box-shadow: 0 0 0 2px var(--qs-blue500)!important; }
    .stSelectbox [data-baseweb="select"] div:focus { box-shadow: 0 0 0 2px rgba(50,154,219,.35)!important; }
    .stSlider [data-baseweb="slider"] div[aria-disabled="false"]:active [role="slider"] { box-shadow: 0 0 0 6px rgba(50,154,219,.25)!important; }
    .stSlider [data-baseweb="slider"] [role="slider"]:focus { outline: none!important; box-shadow: 0 0 0 6px rgba(50,154,219,.25)!important; }
    
    
/* === Cohort Builder: grayscale styling === */
#cohort-builder .panel { 
  border: 2px solid var(--qs-gray-300) !important;
  box-shadow: 0 2px 10px rgba(0,0,0,.05) !important;
  border-radius: 14px !important;
}
#cohort-builder .header-title, 
#cohort-builder .header-sub { color: var(--qs-ink) !important; }

/* Sliders: track & handle in grayscale */
#cohort-builder .stSlider [data-baseweb="slider"]>div>div,
#cohort-builder .stSlider [data-baseweb="slider"]>div>div>div { background: var(--qs-gray-300) !important; }
#cohort-builder .stSlider [role="slider"] { background: var(--qs-gray-700) !important; border-color: var(--qs-gray-700) !important; }
#cohort-builder .stSlider [data-testid="stTickBar"] div { background: var(--qs-gray-300) !important; }
#cohort-builder .stSlider [data-baseweb="slider"]>div>div>div>div { background: var(--qs-gray-300) !important; }
#cohort-builder .stSlider [data-baseweb="slider"] div[aria-disabled="false"]:active [role="slider"] { box-shadow: 0 0 0 6px rgba(16,24,40,.15) !important; }
#cohort-builder .stSlider [data-baseweb="slider"] [role="slider"]:focus { outline: none !important; box-shadow: 0 0 0 6px rgba(16,24,40,.15) !important; }

/* Radios (Include/Exclude etc.) in grayscale */
#cohort-builder .stRadio [role="radio"][aria-checked="true"] { box-shadow: 0 0 0 2px var(--qs-gray-700) !important; }

/* Pills/tags (multiselect) keep neutral borders */
#cohort-builder .stMultiSelect [data-baseweb="tag"] { background: var(--qs-gray-100) !important; color: var(--qs-ink) !important; border-color: var(--qs-gray-300) !important; }

/* Keep primary buttons blue globally, do not change .stButton */

    
/* Distinguish panels A and B with subtle grayscale backgrounds */
#cohort-builder .panel-a { background: #FFFFFF !important; }
#cohort-builder .panel-b { background: var(--qs-gray-50) !important; }
#cohort-builder .panel-a .header-title::before { content: "A"; display:inline-flex; align-items:center; justify-content:center; width:22px; height:22px; border-radius:999px; background:#000; color:#fff; font-weight:700; font-size:.75rem; margin-right:8px; }
#cohort-builder .panel-b .header-title::before { content: "B"; display:inline-flex; align-items:center; justify-content:center; width:22px; height:22px; border-radius:999px; background:#000; color:#fff; font-weight:700; font-size:.75rem; margin-right:8px; }

    
/* === Cohort Builder (scoped) ================================== */
#cohort-builder { --primary-color: var(--qs-ink); }

/* Cards */
#cohort-builder .cohort-card {
  border: 2px solid var(--qs-gray-300) !important;
...
#cohort-builder .panel-a { background: #fff !important; }
#cohort-builder .panel-b { background: var(--qs-gray-50) !important; }

/* Column divider */
#cohort-builder [data-testid="stHorizontalBlock"] { position: relative; }
#cohort-builder [data-testid="stHorizontalBlock"]::after {
...
/* Grayscale sliders */
#cohort-builder .stSlider [data-baseweb="slider"]>div>div,
#cohort-builder .stSlider [data-baseweb="slider"]>div>div>div,
#cohort-builder .stSlider [data-baseweb="slider"]>div>div>div>div { background: var(--qs-gray-300) !important; }
#cohort-builder .stSlider [role="slider"] { background: var(--qs-gray-700) !important; border-color: var(--qs-gray-700) !important; }
#cohort-builder .stSlider [data-testid="stTickBar"] div { background: var(--qs-gray-300) !important; }

/* Radios (no red) */
#cohort-builder input[type="radio"] { accent-color: #111 !important; }

/* Multiselect tags */
#cohort-builder [data-baseweb="tag"] { background: var(--qs-gray-100) !important; color: var(--qs-ink) !important; border-color: var(--qs-gray-300) !important; }
#cohort-builder [data-baseweb="tag"] svg { color: var(--qs-gray-700) !important; }
...
def filter_tests_in_window(tests: pd.DataFrame, win: pd.DataFrame, cond: Dict) -> pd.DataFrame:
    ...
    include_start = bool(cond.get("include_start", True))
    include_end = bool(cond.get("include_end", True))
    left_ok = d["test_date"] >= d["win_start"] if include_start else d["test_date"] > d["win_start"]
    right_ok = (d["win_end"].isna()) | (d["test_date"] <= d["win_end"]) if include_end else (d["win_end"].isna()) | (d["test_date"] < d["win_end"])
    d = d[left_ok & right_ok].copy()

    res_list = cond.get("results", [])
    if res_list:
...
def cohorts_tab(tab):
    with tab:
        ...
        # Wrapper for cohort-only styling
        st.markdown('<div id="cohort-builder">', unsafe_allow_html=True)

        left, right = st.columns(2)
        with left:
            st.markdown('<div class="panel cohort-card panel-a">', unsafe_allow_html=True)
            A = build_cohort_ui("A")
            st.markdown('</div>', unsafe_allow_html=True)
        with right:
            st.markdown('<div class="panel cohort-card panel-b">', unsafe_allow_html=True)
            B = build_cohort_ui("B")
            st.markdown('</div>', unsafe_allow_html=True)
        ...
        if st.button("Reset both cohorts"):
            for p in ["A","B"]:
                for k in [f"{p}_built", f"{p}_label_built", f"{p}_filters_built", f"{p}_rules_built", f"{p}_logic_built", f"{p}_rules"]:
                    if k in st.session_state: del st.session_state[k]
        ...
        st.session_state["cohortA"] = A
        st.session_state["cohortB"] = B
...
def km_stats_tab(tab):
    ...
    if not cfg:
        st.info("Set an endpoint in the **Endpoint & Model** tab.")
        return
...
# === QS KM APP: clean overrides for helper functions ===
# These replace earlier versions to fix syntax/indentation issues.

def window_for_condition(patients: pd.DataFrame, cond: Dict) -> pd.DataFrame:
    start_anchor = cond.get("start_anchor", "diagnosis_date")
    start = pd.to_datetime(patients.get(start_anchor), errors="coerce")
    start_offset = int(cond.get("start_offset", 0) or 0)
    if start_offset:
        start = start + pd.to_timedelta(start_offset, unit="D")

    end_mode = cond.get("end_mode", "anchor")
    if end_mode == "anchor":
        end_anchor = cond.get("end_anchor", "last_followup_date")
        end = pd.to_datetime(patients.get(end_anchor), errors="coerce")
        end_offset = int(cond.get("end_offset", 0) or 0)
        if end_offset:
            end = end + pd.to_timedelta(end_offset, unit="D")
    else:
        end = start + pd.to_timedelta(int(cond.get("end_offset", 0) or 0), unit="D")
...
def eval_anchor_exists(patients: pd.DataFrame, target: str, should_exist: bool) -> pd.Series:
    series = pd.to_datetime(patients.get(target), errors="coerce")
    exists = series.notna()
    return exists if should_exist else ~exists

def eval_anchor_order(patients: pd.DataFrame, left: str, op: str, right: str, miss_left: str, miss_right: str) -> pd.Series:
    L = pd.to_datetime(patients.get(left), errors="coerce")
    R = pd.to_datetime(patients.get(right), errors="coerce")
    l_missing = L.isna()
    r_missing = R.isna()
    ...
    if miss_left == "pass": base[l_missing] = True
    if miss_right == "pass": base[r_missing] = True
    return base.fillna(False)
