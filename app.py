...
/* Force visible tab labels */
div[role="tablist"] button[role="tab"] {
  color: var(--qs-blue900) !important;
  opacity: 1 !important;
}
div[role="tablist"] button[role="tab"][aria-selected="true"] {
  color: var(--qs-blue500) !important;
  border-bottom: 2px solid var(--qs-blue500) !important;
}

</style>
"""

# Minimal UI mode switch (set in Data tab)
minimal_mode = st.session_state.get("minimal_ui", False)

# Rebuild CSS depending on minimal_mode
if minimal_mode:
    CUSTOM_CSS = CUSTOM_CSS.replace("position: sticky; top: 0; z-index: 2;", "position: static;")
    CUSTOM_CSS = CUSTOM_CSS.replace("box-shadow: 0 2px 16px rgba(0,0,0,0.06);", "box-shadow: none;")
    CUSTOM_CSS = CUSTOM_CSS.replace("background: var(--qs-blue100);", "background: white;")
...
def data_tab():
    # Frontend health & UI mode
    with tabs[0]:
        hc1, hc2 = st.columns([1,1])
        with hc1:
            st.checkbox("Minimal UI mode (turn off sticky header/CSS effects)", key="minimal_ui")
            if st.button("Apply UI mode"):
                st.rerun()
        with hc2:
            import streamlit as _st
            st.caption(f"Streamlit version: {_st.__version__}")
            st.checkbox("High-contrast tab labels", key="hc_tabs", value=True)
