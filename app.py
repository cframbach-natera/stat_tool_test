...
st.set_page_config(page_title="QS Oncology Stats (POC v5.1.3)", page_icon="🧬", layout="wide")
...
def render_css(minimal: bool=False, hc_tabs: bool=True):
...
# ---------------- Tabs content ----------------
def data_tab(tab, minimal_default=False, hc_tabs_default=True):
    with tab:
        ...
        with c0:
            st.checkbox("Minimal UI mode (turn off sticky header/CSS effects)", key="minimal_ui", value=st.session_state.get("minimal_ui", minimal_default))
            if st.button("Apply UI mode"):
                st.rerun()
        with c00:
            st.checkbox("High-contrast tab labels", key="hc_tabs", value=st.session_state.get("hc_tabs", hc_tabs_default))
    ...
    with tab:
        ...
        with st.expander("📊 Statistics"):
            ...
            try:
                ...
                import numpy as _np
                hr = float(_np.exp(cph.params_["group"]))
                ci = cph.confidence_intervals_.loc["group"].values
                st.write(f"Cox PH HR (B vs A): **{hr:.3g}**  [{_np.exp(ci[0]):.3g}, {_np.exp(ci[1]):.3g}]")
            except Exception as e:
...
def main():
    minimal = st.session_state.get("minimal_ui", False)
    hc_tabs = st.session_state.get("hc_tabs", True)
    render_css(minimal=minimal, hc_tabs=hc_tabs)

    tabs = make_tabs()
    with tabs[0]:
        st.markdown('<div class="header-bar"><div class="header-title">QS Oncology Stats (POC v5.1.3)</div><div class="header-sub">Flexible rule builder • Tests+Patients • OS/TTP/PFS/TTT/TTNT</div></div>', unsafe_allow_html=True)
    data_tab(tabs[0])
    cohorts_tab(tabs[1])
    endpoint_tab(tabs[2])
    km_stats_tab(tabs[3])
