...
def km_stats_tab(tab):
    with tab:
        ...
        def mask_from_rules(pat_df, rules, logic):
            ...
            comb = pd.concat(masks, axis=1)
            return comb.any(axis=1) if logic == "ANY" else comb.all(axis=1)
        ...
        kfA.plot(ax=ax, ci_show=True, color=BLUE)
        kfB.plot(ax=ax, ci_show=True, color=GREEN)

        # Manual censor ticks
        try:
            cA = kmA.loc[kmA["km_event"]==0, "km_time"].astype(float).values
            yA = kfA.predict(cA).astype(float).values
            if len(cA) > 0:
                ax.vlines(cA, yA-0.012, yA+0.012, colors=BLUE, linewidth=1, alpha=0.9)
            cB = kmB.loc[kmB["km_event"]==0, "km_time"].astype(float).values
            yB = kfB.predict(cB).astype(float).values
            if len(cB) > 0:
                ax.vlines(cB, yB-0.012, yB+0.012, colors=GREEN, linewidth=1, alpha=0.9)
        except Exception:
            pass
        ...
        with st.expander("📊 Statistics"):
            ...
            tps_months = cfg.get("km_tp_months", [12,24,36])
            unit = cfg.get("unit","months")
            def tp_to_unit(m):
