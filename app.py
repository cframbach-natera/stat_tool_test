...
st.set_page_config(page_title="QS Oncology Stats (Builder v5.2)", page_icon="🧬", layout="wide")
...
# ---------------- CSS (tabs visible, no sticky header by default) ----------------
def render_css(high_contrast_tabs: bool=True):
    base_css = f"""
    <style>
    :root {{
    ...
    tabs_css = """
    <style>
    div[role="tablist"] button[role="tab"] { color: #1B5F7F !important; opacity: 1 !important; }
    div[role="tablist"] button[role="tab"][aria-selected="true"] {
        color: #329ADB !important; border-bottom: 2px solid #329ADB !important;
    }
    </style>
    """ if high_contrast_tabs else ""
    st.markdown(base_css + tabs_css, unsafe_allow_html=True)
...
# ---------------- Rule Builder ----------------
"""
We support two families of conditions:

A) Tests-in-window conditions (same as before):
   - Count, Exists, None, FirstResult, LastResult, Consecutive, Proportion

B) NEW patient-anchor conditions (no tests needed):
   - AnchorExists:  require a given anchor (date) to exist or not
   - AnchorOrder:   compare two anchors with <, <=, >, >=, ==, !=
                    plus a policy for what to do if either side is missing
"""
...
def render_condition(prefix: str, idx: int, tests_df: Optional[pd.DataFrame]):
    ...
    if fam == "tests":
        ...
        if cond["anchor_type"] == "AnchorExists":
            ...
            with c2:
                cond["anchor_exists_should"] = st.selectbox("Should", [True, False],
...
def eval_anchor_order(patients: pd.DataFrame, left: str, op: str, right: str,
    ...
    # if both missing -> fail (conservative)
...
def data_tab(tab):
    with tab:
        ...
        st.checkbox("High-contrast tab labels", value=True, key="hc_tabs")
        render_css(high_contrast_tabs=st.session_state.get("hc_tabs", True))
...
def cohorts_tab(tab):
    with tab:
        st.markdown('<div class="header-bar"><div class="header-title">Cohorts</div><div class="header-sub">All relationships are buildable via rules (no flags).</div></div>', unsafe_allow_html=True)
...
def km_stats_tab(tab):
    with tab:
        ...
        # Apply quick filters; then evaluate rules
        dA_q = apply_quick_filters(patients, A["qf"])
        dB_q = apply_quick_filters(patients, B["qf"])

        def apply_with_snapshot(df, snap):
            live_rules = st.session_state.get(f"tmp_rules", None)
            live_logic = st.session_state.get(f"tmp_logic", None)
            try:
                st.session_state["tmp_rules"] = snap["rules"]
                st.session_state["tmp_logic"] = snap["logic"]
                st.session_state["A_rules"] = snap["rules"]  # not used elsewhere; safe stash
                st.session_state["A_logic"] = snap["logic"]
                st.session_state["rules"] = snap["rules"]
                st.session_state["rules_logic"] = snap["logic"]
                # Temporarily evaluate using snap
                ...
                if live_rules is not None: st.session_state["tmp_rules"] = live_rules
                if live_logic is not None: st.session_state["tmp_logic"] = live_logic
...
def main():
    render_css(high_contrast_tabs=True)
    tabs = make_tabs()
    with tabs[0]:
        st.markdown('<div class="header-bar"><div class="header-title">QS Oncology Stats – Cohort Builder v5.2</div><div class="header-sub">Build everything in rules (no flags). Tabs are high-contrast and always visible.</div></div>', unsafe_allow_html=True)
    data_tab(tabs[0])
    cohorts_tab(tabs[1])
    endpoint_tab(tabs[2])
    km_stats_tab(tabs[3])
