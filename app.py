...
def render_css():
    ...
    :root { --qs-blue:#60A4BF; --qs-blue500:#329ADB; --qs-blue900:#1B5F7F; --qs-green:#7EB86E;
            ...
            --qs-top-offset: 84px; }
    .block-container{padding-top:calc(var(--qs-top-offset) + 8px);padding-bottom:2rem;}
...
def endpoint_tab(tab):
    ...
    with tab:
        ...
        # New: KM reporting timepoints (months)
        st.markdown("### KM reporting timepoints")
        tp_str = st.text_input("Timepoints (months, comma-separated)", value=st.session_state.get("km_tp_default","12,24,36"))
        try:
            tps = [int(x.strip()) for x in tp_str.split(",") if x.strip()!=""]
        except Exception:
            tps = [12,24,36]
        st.session_state["km_tp_default"] = ",".join(str(x) for x in tps)
        st.session_state["endpoint_cfg"]={"type":ev_type,"baseline":baseline,"unit":unit,"km_tp_months": tps}
...
def km_stats_tab(tab):
    with tab:
        ...
        def mask_from_rules(pat_df, rules, logic):
            if not rules: return pd.Series(True, index=pat_df.index)
            masks = []
            for cond in rules:
                try:
            ...
            return pd.concat(masks, axis=1).any(axis=1) if logic == "ANY" else pd.concat(masks, axis=1).all(axis=1)
        ...
        subA = dA_q.loc[mA].copy(); subB = dB_q.loc[mB].copy()
        if subA.empty or subB.empty:
            st.info(f"Need >0 patients in each cohort. A={{len(subA)}}, B={{len(subB)}}."); return

        kmA = compute_km_dataset(subA, tests, cfg); kmB = compute_km_dataset(subB, tests, cfg)
        ...
        lineA = kfA.plot(ax=ax, ci_show=True)
        lineB = kfB.plot(ax=ax, ci_show=True)
        try:
            cA = kmA.loc[kmA["km_event"]==0, "km_time"]; yA = kfA.predict(cA).values
            cB = kmB.loc[kmB["km_event"]==0, "km_time"]; yB = kfB.predict(cB).values
            ax.scatter(cA, yA, marker='|', s=100, linewidths=1, color=lineA[0].get_color())
            ax.scatter(cB, yB, marker='|', s=100, linewidths=1, color=lineB[0].get_color())
        except Exception:
            pass
        ax.set_xlabel(kmA["km_time_label"].iloc[0]); ax.set_ylabel("Survival probability")
        ...
        with st.expander("📊 Statistics"):
            ...
            tps_months = st.session_state.get("endpoint_cfg", {}).get("km_tp_months", [12,24,36])
            unit = st.session_state.get("endpoint_cfg", {}).get("unit","months")
            def tp_to_unit(m):
                if unit == "months": return float(m)
                if unit == "years": return float(m)/12.0
                return float(m)*30.4375
            rows = []
            for m in tps_months:
                t_eval = tp_to_unit(m)
                nA = int((kmA["km_time"] >= t_eval).sum()); nB = int((kmB["km_time"] >= t_eval).sum())
                SA = float(kfA.predict(t_eval)); SB = float(kfB.predict(t_eval))
                rows.append({"months": int(m), "A_n_at_risk": nA, "A_S(t)": SA, "B_n_at_risk": nB, "B_S(t)": SB})
            st.dataframe(pd.DataFrame(rows).set_index("months"))
