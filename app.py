# app.py
# QS Oncology Stats – Cohort Builder v5.2.1 (neutral theme, include/exclude filters, green Build button)
...
# ---------------- CSS (neutral theme; green-only Build button) ----------------
def render_css():
    base = """
    <style>
    :root {
    ...
    /* Tabs: neutral, always visible */
    div[role="tablist"] button[role="tab"] {
      color: var(--qs-ink) !important; opacity: 1 !important; font-weight: 600 !important;
      background: transparent !important; border-bottom: 2px solid transparent !important;
    }
    div[role="tablist"] button[role="tab"][aria-selected="true"] {
      color: var(--qs-blue900) !important; border-bottom-color: var(--qs-blue500) !important;
    }
    ...
    .header-sub { color: var(--qs-gray-700); font-size: 0.9rem; font-weight: 500; }

    /* Panel cards */
    .panel {
      padding: 12px 14px; border-radius: 14px;
      border: 1px solid var(--qs-gray-200); background: #fff;
      box-shadow: 0 1px 6px rgba(0,0,0,0.04);
    }

    /* KPI cards */
    .kpi-card { border-radius: 14px; padding: 12px 16px; background:#fff;
                border: 1px solid var(--qs-gray-200); box-shadow: 0 1px 10px rgba(0,0,0,0.04); }
    .kpi-title { font-weight: 600; color: var(--qs-ink); }
    .kpi-value { font-size: 1.15rem; font-weight: 800; color: var(--qs-blue900); }
    .small { font-size: 0.85rem; color: var(--qs-gray-500); }
    ...
    .stButton > button {
      background-color: var(--qs-blue500) !important;
      border: 1px solid var(--qs-blue900) !important;
      color: #fff !important; font-weight: 700 !important; border-radius: 10px !important;
    }
    .stButton > button:hover { filter: brightness(0.98); }

    /* Scoped "green" buttons */
    .btn-green .stButton > button {
      background-color: var(--qs-green) !important;
      border: 1px solid #5f9e53 !important;
      color: #0b2610 !important;
    }

    /* Multiselect chips – neutral */
    .stMultiSelect [data-baseweb="tag"] {
      background: var(--qs-gray-100) !important;
      color: var(--qs-ink) !important;
      border-radius: 10px !important;
      border: 1px solid var(--qs-gray-300) !important;
      box-shadow: none !important;
    }
    .stMultiSelect [data-baseweb="tag"]:hover { border-color: var(--qs-blue500) !important; }
    .stMultiSelect [data-baseweb="tag"] svg { color: var(--qs-gray-500) !important; }

    /* Dropdown option states */
    .stMultiSelect [data-baseweb="select"] [role="option"][aria-selected="true"] {
      background: var(--qs-gray-100) !important; color: var(--qs-ink) !important;
    }
    .stMultiSelect [data-baseweb="select"] [role="option"]:hover {
      background: var(--qs-gray-50) !important;
    }

    /* Slider / radios / checkboxes -> brand accents */
    ...
    /* Condition cards */
    .condition-card {
      border: 1px dashed var(--qs-gray-300); border-radius: 12px; padding: 10px; margin-bottom: 8px; background: #fff;
    }
    </style>
    """
...
# ---------------- Quick filters with Include/Exclude ----------------
def render_quick_filters(prefix: str, dfp: pd.DataFrame) -> Dict:
    ...
    if "age_at_diag" in dfp.columns:
        ...
        with c1:
            rng = st.slider("Age at diagnosis", min_value=a_min, max_value=a_max,
                            value=(default_rng[0], default_rng[1]), key=key_rng)
    ...
    # Helper to render include/exclude categorical filters
    def cat_filter(col: str, title: str, container):
        if col not in dfp.columns: return
        opts = sorted([x for x in dfp[col].dropna().unique().tolist()], key=lambda x: str(x))
        key_vals = f"{prefix}_{col}_vals"
        key_mode = f"{prefix}_{col}_mode"
        default_vals = st.session_state.get(key_vals, opts)
        default_mode = st.session_state.get(key_mode, "Include")
        with container:
            sel = st.multiselect(title, options=opts, default=default_vals, key=key_vals)
            mode = st.radio("Mode", ["Include","Exclude"], horizontal=True, key=key_mode,
                            index=0 if default_mode=="Include" else 1)
        # Store only if not selecting 'all' in include mode (that's a no-op)
        if sel or mode == "Exclude":
            filters[col] = {"type":"set","values": set(sel), "include": (mode=="Include")}

    # Render categoricals
    cat_filter("sex", "Biological sex", c1)
    cat_filter("race", "Race", c2)
    cat_filter("ethnicity", "Ethnicity", c2)
    cat_filter("stage_group", "Stage group", c3)
    cat_filter("cancer_type", "Cancer type", c3)
...
A) tests-in-window conditions:
   - Count, Exists, None, FirstResult, LastResult, Consecutive, Proportion

B) patient-anchor conditions (no tests required):
   - AnchorExists:  require a given anchor (date) to exist or not
   - AnchorOrder:   compare two anchors with <, <=, >, >=, ==, !=
                    with a policy for handling missing anchors
"""
RESULT_VALUES = ["positive","negative","indeterminate"]
ANCHOR_COMPARE_OPS = ["<", "<=", ">", ">=", "==", "!="]
MISSING_POLICY = ["fail", "pass"]  # if the compared anchor is missing, treat as fail/pass
...
def add_condition(prefix: str):
    default_cond = {
        ...
        "min_tests": 1,
        "facet_filters": {},
        # anchor family defaults:
        "anchor_type": "AnchorExists",          # AnchorExists or AnchorOrder
        "anchor_exists_target": "first_treatment_date",
        "anchor_exists_should": True,           # True = must exist, False = must NOT exist
        "anchor_left": "first_treatment_date",
        "anchor_right": "progression_date",
        "anchor_op": "<",
        "missing_left_policy": "fail",
        "missing_right_policy": "fail",
    }
...
def render_condition(prefix: str, idx: int, tests_df: Optional[pd.DataFrame]):
    ...
    fam = st.radio("Condition family", ["tests", "anchor"], horizontal=True,
                   index=0 if cond.get("family","tests")=="tests" else 1, key=f"{prefix}_fam_{idx}")
    cond["family"] = fam
    ...
    if fam == "tests":
        ...
        if cond["type"] in ["FirstResult","LastResult"]:
            ...
            cond["include_indeterminate"] = st.checkbox("Include indeterminate in pool",
                value=bool(cond["include_indeterminate"]), key=f"{prefix}_ind2_{idx}")
            if cond["type"] == "Count":
        ...
        # ------- anchor-order / exists UI -------
        cond["anchor_type"] = st.selectbox("Anchor condition", ["AnchorExists","AnchorOrder"],
                                           index=["AnchorExists","AnchorOrder"].index(cond.get("anchor_type","AnchorExists")),
                                           key=f"{prefix}_anchtype_{idx}")
        if cond["anchor_type"] == "AnchorExists":
            c1, c2 = st.columns(2)
            with c1:
                cond["anchor_exists_target"] = st.selectbox("Anchor", PATIENT_ANCHORS,
                    index=PATIENT_ANCHORS.index(cond.get("anchor_exists_target","first_treatment_date")),
                    key=f"{prefix}_axt_{idx}")
            with c2:
                cond["anchor_exists_should"] = st.selectbox("Should exist?", [True, False],
                    index=0 if cond.get("anchor_exists_should", True) else 1,
                    key=f"{prefix}_axs_{idx}")
                st.caption("True = must exist; False = must NOT exist.")
        else:
            c1, c2, c3 = st.columns([1,0.5,1])
            with c1:
                cond["anchor_left"] = st.selectbox("Left anchor", PATIENT_ANCHORS,
                    index=PATIENT_ANCHORS.index(cond.get("anchor_left","first_treatment_date")), key=f"{prefix}_al_{idx}")
            with c2:
                cond["anchor_op"] = st.selectbox("Operator", ANCHOR_COMPARE_OPS,
                    index=ANCHOR_COMPARE_OPS.index(cond.get("anchor_op","<")), key=f"{prefix}_aop_{idx}")
            with c3:
                cond["anchor_right"] = st.selectbox("Right anchor", PATIENT_ANCHORS,
                    index=PATIENT_ANCHORS.index(cond.get("anchor_right","progression_date")), key=f"{prefix}_ar_{idx}")
            d1, d2 = st.columns(2)
            with d1:
                cond["missing_left_policy"] = st.selectbox("If left missing", MISSING_POLICY,
                    index=MISSING_POLICY.index(cond.get("missing_left_policy","fail")), key=f"{prefix}_mlp_{idx}")
            with d2:
                cond["missing_right_policy"] = st.selectbox("If right missing", MISSING_POLICY,
                    index=MISSING_POLICY.index(cond.get("missing_right_policy","fail")), key=f"{prefix}_mrp_{idx}")
...
def eval_anchor_exists(patients: pd.DataFrame, target: str, should_exist: bool) -> pd.Series:
    series = pd.to_datetime(patients[target], errors="coerce")
    exists = series.notna()
    return exists if should_exist else ~exists

def eval_anchor_order(patients: pd.DataFrame, left: str, op: str, right: str,
                      miss_left: str, miss_right: str) -> pd.Series:
    L = pd.to_datetime(patients[left], errors="coerce")
    R = pd.to_datetime(patients[right], errors="coerce")
    l_missing = L.isna()
    r_missing = R.isna()
    base = pd.Series(False, index=patients.index)
    both = ~(l_missing | r_missing)
...
def eval_condition(patients: pd.DataFrame, tests: pd.DataFrame, cond: Dict) -> pd.Series:
    if cond.get("family","tests") == "anchor":
        if cond.get("anchor_type","AnchorExists") == "AnchorExists":
            ...
            return eval_anchor_order(patients,
                                     cond["anchor_left"], cond["anchor_op"], cond["anchor_right"],
                                     cond.get("missing_left_policy","fail"), cond.get("missing_right_policy","fail"))
    # tests family
...
def apply_rules(prefix: str, patients: pd.DataFrame, tests: pd.DataFrame) -> pd.DataFrame:
    ...
    masks = [eval_condition(patients, tests, cond) for cond in rules]
    if logic == "ALL":
...
def apply_quick_filters(patients: pd.DataFrame, filters: Dict) -> pd.DataFrame:
    """Supports include/exclude for categoricals and range inversion for age."""
    if not filters:
        return patients.copy()
    d = patients.copy()
    mask = pd.Series(True, index=d.index)
    for col, spec in filters.items():
        # Backward compatibility if old tuple format ever shows up
        if isinstance(spec, tuple):
            ftype, val = spec[0], spec[1]
            include = True
            if ftype == "range":
                lo, hi = val
                mask &= d[col].astype(float).between(lo, hi)
            elif ftype in ("in", "set"):
                mask &= d[col].isin(val)
            continue

        ftype = spec.get("type")
        include = spec.get("include", True)

        if ftype == "range":
            lo, hi = spec["value"]
            inside = d[col].astype(float).between(lo, hi)
            mask &= inside if include else ~inside
        elif ftype in ("set", "in"):
            vals = spec.get("values", set())
            in_set = d[col].isin(vals)
    ...
    with col2:
        if st.button(f"Reset {prefix}", key=f"{prefix}_reset"):
            for k in [f"{prefix}_built", f"{prefix}_label_built", f"{prefix}_qf_built",
                      f"{prefix}_rules_built", f"{prefix}_logic_built", f"{prefix}_rules"]:
        ...
        if st.button("Reset both cohorts"):
            for p in ["A","B"]:
                for k in [f"{p}_built", f"{p}_label_built", f"{p}_qf_built",
                          f"{p}_rules_built", f"{p}_logic_built", f"{p}_rules"]:
                    if k in st.session_state: del st.session_state[k]
...
def km_stats_tab(tab):
    with tab:
        ...
        # Apply quick filters; then evaluate rules using snapshots saved at build time
        ...
        dA = apply_with_snapshot(dA_q, A)
        dB = apply_with_snapshot(dB_q, B)

        if dA.empty or dB.empty:
            st.warning("One cohort is empty after filters/rules."); return
        ...
        for i, (lab, d) in enumerate([(A["label"], mA), (B["label"], mB)]):
            try:
                ...
                med = np.nan
            med_txt = "NA" if (not np.isfinite(med)) else f"{med:.2f}"
            rate = f"{(d['km_event'].sum()/len(d)*100):.0f}%" if len(d) else "—"
            html = f"""
            <div class="kpi-card">
              <div class="kpi-title">{lab}</div>
              <div class="kpi-value">Median: {med_txt}</div>
              <div class="small">N={len(d)} • Events={int(d['km_event'].sum())} ({rate})</div>
            </div>
            """
        ...
        for (lab, d), color in zip([(A["label"], mA), (B["label"], mB)], colors):
            ...
            try:
                ...
                st.warning(f"KM error for {lab}: {e}")
        ...
        with st.expander("📊 Statistics"):
            ...
            try:
                tmp = pd.concat([mA.assign(group=0), mB.assign(group=1)], ignore_index=True)
                cph = CoxPHFitter()
