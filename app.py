...
st.set_page_config(page_title="QS Oncology Stats (POC v5.1.4)", page_icon="🧬", layout="wide")
...
def render_css(minimal: bool=False):
    header_pos = "" if minimal else "position: sticky; top: 0; z-index: 2;"
    header_bg = "white" if minimal else "var(--qs-blue100)"
    header_shadow = "" if minimal else "box-shadow: 0 2px 16px rgba(0,0,0,0.06);"
    base_css = f"""
    <style>
    :root {{
    ...
    div[role="tablist"] button[role="tab"] {{ color: var(--qs-blue900) !important; opacity: 1 !important; }}
    div[role="tablist"] button[role="tab"][aria-selected="true"] {{ color: var(--qs-blue500) !important; border-bottom: 2px solid var(--qs-blue500) !important; }}
    </style>
    """
...
# ---------------- Data loaders ----------------
PATIENT_ANCHORS = [
    "diagnosis_date","surgery_date","first_treatment_date","progression_date",
    "next_treatment_date","death_date","last_followup_date"
]
...
def render_quick_filters(prefix: str, dfp: pd.DataFrame) -> Dict:
    ...
    if "age_at_diag" in dfp.columns:
        ...
        key = f"{prefix}_age_range"; default = st.session_state.get(key, (a_min, a_max))
        with c1:
    ...
    for col, col_title, col_container in [
        ...
        if col in dfp.columns:
            ...
            key = f"{prefix}_{col}_in"; default = st.session_state.get(key, opts)
            with col_container:
...
def init_rules(prefix: str):
    ...
    # NEW: patient-level toggles used to enforce anchor relationships
    st.session_state.setdefault(f"{prefix}_pat_rules", {
        "require_surgery": False,
        "require_progression": False,
        "require_treatment": True,
        "tx_before_prog": False,               # for Cohort A
        "tx_after_prog": False,                # for Cohort B
        "first_pos_after_surg_before_prog": False  # for Cohort B
    })
...
def render_condition(prefix: str, idx: int, tests_df: pd.DataFrame):
    ...
    with c1:
        cond["type"] = st.selectbox("Condition type",
            ["Count","Exists","None","FirstResult","LastResult","Consecutive","Proportion"],
            index=["Count","Exists","None","FirstResult","LastResult","Consecutive","Proportion"].index(cond["type"]),
            key=f"{prefix}_type_{idx}")
    with c2:
        cond["start_anchor"] = st.selectbox("Start anchor", PATIENT_ANCHORS,
            index=PATIENT_ANCHORS.index(cond["start_anchor"]), key=f"{prefix}_sa_{idx}")
    ...
    with c3:
        ...
        if cond["end_mode"] == "anchor":
            cond["end_anchor"] = st.selectbox("End anchor", PATIENT_ANCHORS,
                index=PATIENT_ANCHORS.index(cond["end_anchor"]), key=f"{prefix}_ea_{idx}")
        else:
    ...
    if cond["type"] in ["FirstResult","LastResult"]:
        cond["require_result"] = st.selectbox("Required result", RESULT_VALUES,
            index=RESULT_VALUES.index(cond["require_result"]), key=f"{prefix}_req_{idx}")
        cond["include_indeterminate"] = st.checkbox("Consider indeterminate in ordering",
            value=bool(cond["include_indeterminate"]), key=f"{prefix}_ind_{idx}")
        ...
        cond["include_indeterminate"] = st.checkbox("Include indeterminate in pool",
            value=bool(cond["include_indeterminate"]), key=f"{prefix}_ind_{idx}")
        if cond["type"] == "Count":
            cond["count_op"] = st.selectbox("Comparator", ["at least","at most","exactly"],
                index=["at least","at most","exactly"].index(cond["count_op"]), key=f"{prefix}_cop_{idx}")
...
def filter_tests_in_window(tests: pd.DataFrame, win: pd.DataFrame, cond: Dict) -> pd.DataFrame:
    ...
    if cond["type"] in ["Count","Exists","None","Consecutive","Proportion"]:
        if cond["results"]:
            ...
            if cond["include_indeterminate"]:
                res_set.add("indeterminate")
...
def eval_condition(patients: pd.DataFrame, tests: pd.DataFrame, cond: Dict) -> pd.Series:
    ...
    if cond["type"] == "Consecutive":
        ...
        if k <= 1:
            ...
            return patients["patient_id"].map(has_any).fillna(False)
        ok_ids = set()
...
# -------- Patient-level rules (anchors + helper using tests) --------
def first_positive_after_surgery_before_progression(patients: pd.DataFrame, tests: pd.DataFrame) -> pd.Series:
    """First positive test date after surgery, and that test occurs before progression."""
    t = tests[tests["result"]=="positive"].copy()
    merged = t.merge(patients[["patient_id","surgery_date","progression_date"]], on="patient_id", how="left")
    m = (~merged["surgery_date"].isna()) & (merged["test_date"] >= merged["surgery_date"])
    merged = merged[m]
    first_pos = merged.sort_values(["patient_id","test_date"]).groupby("patient_id").first()["test_date"]
    fp = patients["patient_id"].map(first_pos)
    prog = pd.to_datetime(patients["progression_date"], errors="coerce")
    return (fp.notna()) & (prog.notna()) & (fp < prog)

def apply_patient_rules(prefix: str, patients: pd.DataFrame, tests: pd.DataFrame) -> pd.DataFrame:
    rules = st.session_state.get(f"{prefix}_pat_rules", {})
    d = patients.copy()

    if rules.get("require_surgery", False):
        d = d[~d["surgery_date"].isna()]
    if rules.get("require_progression", False):
        d = d[~d["progression_date"].isna()]
    if rules.get("require_treatment", True):
        d = d[~d["first_treatment_date"].isna()]

    if rules.get("tx_before_prog", False):
        # allow missing progression, or tx strictly before progression
        d = d[(d["progression_date"].isna()) | (d["first_treatment_date"] < d["progression_date"])]
    if rules.get("tx_after_prog", False):
        # require progression exists and is strictly before treatment
        d = d[(~d["progression_date"].isna()) & (d["first_treatment_date"] > d["progression_date"])]

    if rules.get("first_pos_after_surg_before_prog", False):
        mask = first_positive_after_surgery_before_progression(d, tests)
        d = d[mask]
...
def apply_rules(prefix: str, patients: pd.DataFrame, tests: pd.DataFrame) -> pd.DataFrame:
    # Patient-level first
    d = apply_patient_rules(prefix, patients, tests)
    # Then tests-based conditions
    rules: List[Dict] = st.session_state.get(f"{prefix}_rules", [])
    logic = st.session_state.get(f"{prefix}_rules_logic","ALL")
    if not rules:
        return d
    masks = [eval_condition(d, tests, cond) for cond in rules]
    if logic == "ALL":
...
def data_tab(tab):
    with tab:
        ...
        with c0:
            st.checkbox("Minimal UI mode (turn off sticky header/CSS effects)", key="minimal_ui", value=st.session_state.get("minimal_ui", False))
            if st.button("Apply UI mode"):
                st.rerun()
        with c00:
            if st.button("Clear Streamlit caches"):
                st.cache_data.clear(); st.cache_resource.clear()
                st.success("Cleared data/resource caches.")
        ...
        with c1:
            ...
            if use_pat_sample and pat_file is None:
                ...
                st.download_button("Download patients sample", data=dfp.to_csv(index=False).encode("utf-8"),
                                   file_name="patients_sample.csv", mime="text/csv")
        ...
        with c2:
            ...
            if use_tests_sample and tst_file is None:
                ...
                st.download_button("Download tests sample", data=dft.to_csv(index=False).encode("utf-8"),
                                   file_name="tests_sample.csv", mime="text/csv")
...
# -------- Presets to enforce your definitions --------
def preset_cohort(prefix: str, name: str):
    init_rules(prefix)
    # clear tests rules
    st.session_state[f"{prefix}_rules"] = []
    pr = st.session_state[f"{prefix}_pat_rules"]

    if name == "A":
        # A: surgery → positive test (post-surgery, pre-tx) → tx before any progression
        pr.update({
            "require_surgery": True,
            "require_treatment": True,
            "require_progression": False,
            "tx_before_prog": True,
            "tx_after_prog": False,
            "first_pos_after_surg_before_prog": False,  # not required for A
        })
        # ensure at least one positive between surgery and treatment
        st.session_state[f"{prefix}_rules"].append({
            "type": "Exists",
            "start_anchor": "surgery_date",
            "start_offset": 0,
            "end_mode": "anchor",
            ...
            "end_offset": 90,
            "include_start": True,
            "include_end": False,  # strictly before tx
            "results": ["positive"],
            "require_result": "positive",
            "include_indeterminate": False,
            "count_op": "at least",
        ...
        # B: surgery → (first positive after surgery occurs BEFORE progression) → progression → tx AFTER progression
        pr.update({
            "require_surgery": True,
            "require_treatment": True,
            "require_progression": True,
            "tx_before_prog": False,
            "tx_after_prog": True,
            "first_pos_after_surg_before_prog": True,
        })

    st.session_state[f"{prefix}_rules_logic"] = "ALL"
    st.success(f"Applied preset for Cohort {prefix}: {name}")
...
def build_cohort_ui(prefix: str):
    ...
    with pcol1:
        if st.button("Apply preset: A", key=f"{prefix}_presetA"):
            preset_cohort(prefix, "A")
    with pcol2:
        if st.button("Apply preset: B", key=f"{prefix}_presetB"):
            preset_cohort(prefix, "B")

    # Patient-level toggles
    st.markdown("### Patient-level toggles")
    pr = st.session_state.get(f"{prefix}_pat_rules")
    c1, c2, c3 = st.columns(3)
    with c1:
        pr["require_surgery"] = st.checkbox("Require surgery", value=pr["require_surgery"], key=f"{prefix}_req_surg")
        pr["require_treatment"] = st.checkbox("Require 1L treatment", value=pr["require_treatment"], key=f"{prefix}_req_tx")
        pr["require_progression"] = st.checkbox("Require progression", value=pr["require_progression"], key=f"{prefix}_req_prog")
    with c2:
        pr["tx_before_prog"] = st.checkbox("Treatment BEFORE progression", value=pr["tx_before_prog"], key=f"{prefix}_tx_before_prog")
        pr["tx_after_prog"] = st.checkbox("Treatment AFTER progression", value=pr["tx_after_prog"], key=f"{prefix}_tx_after_prog")
    with c3:
        pr["first_pos_after_surg_before_prog"] = st.checkbox("First positive after surgery occurs BEFORE progression",
                                                             value=pr["first_pos_after_surg_before_prog"],
                                                             key=f"{prefix}_firstpos_before_prog")

    # Tests-based rules
    st.markdown("### Tests-based rules")
    ...
    with col1:
        if st.button(f"Build {label}", type="primary", key=f"{prefix}_build"):
            ...
            st.session_state[f"{prefix}_pat_rules_built"] = st.session_state[f"{prefix}_pat_rules"]
            st.success(f"Cohort {prefix} built.")
    with col2:
        if st.button(f"Reset {prefix}", key=f"{prefix}_reset"):
            for k in [f"{prefix}_built", f"{prefix}_label_built", f"{prefix}_qf_built",
                      f"{prefix}_rules_built", f"{prefix}_logic_built", f"{prefix}_pat_rules_built", f"{prefix}_rules"]:
    ...
    pr_built = st.session_state.get(f"{prefix}_pat_rules_built", st.session_state.get(f"{prefix}_pat_rules", {}))

    # Persist latest edits
    st.session_state[f"{prefix}_pat_rules"] = pr

    return {"label": label_built, "qf": qf_built, "rules": rules_built, "logic": logic_built,
            "pat_rules": pr_built, "built": built}

def cohorts_tab(tab):
    with tab:
        st.markdown('<div class="header-bar"><div class="header-title">Cohorts</div><div class="header-sub">Presets for A/B, patient-level toggles, and test rules.</div></div>', unsafe_allow_html=True)
        ...
        if st.button("Reset both cohorts"):
            for p in ["A","B"]:
                for k in [f"{p}_built", f"{p}_label_built", f"{p}_qf_built",
                          f"{p}_rules_built", f"{p}_logic_built", f"{p}_pat_rules_built", f"{p}_rules"]:
                    if k in st.session_state: del st.session_state[k]
...
def endpoint_tab(tab):
    with tab:
        st.markdown('<div class="header-bar"><div class="header-title">Endpoint & Model</div><div class="header-sub">Choose OS and baseline.</div></div>', unsafe_allow_html=True)
        patients = st.session_state.get("patients_df")
        tests = st.session_state.get("tests_df")
        if patients is None or tests is None:
            st.info("Load data in the **Data** tab first."); return
        ...
        ], index=0, key="ev_type")  # default OS
        ...
        ], index=2, key="baseline_choice")  # default Surgery
...
def km_stats_tab(tab):
    with tab:
        st.markdown('<div class="header-bar"><div class="header-title">KM & Stats</div><div class="header-sub">OS by cohort; at-risk + stats.</div></div>', unsafe_allow_html=True)
        patients = st.session_state.get("patients_df"); tests = st.session_state.get("tests_df")
        if patients is None or tests is None:
            st.info("Load data in the **Data** tab first."); return
        ...
        dA_q = apply_quick_filters(patients, A["qf"])
        dB_q = apply_quick_filters(patients, B["qf"])

        # Apply using the snapshots saved at build time
        def apply_with_snapshot(prefix, df, snapshot):
            # stash live
            live_pat = st.session_state.get(f"{prefix}_pat_rules", None)
            live_rules = st.session_state.get(f"{prefix}_rules", None)
            live_logic = st.session_state.get(f"{prefix}_rules_logic", None)
            try:
                st.session_state[f"{prefix}_pat_rules"] = snapshot["pat_rules"]
                st.session_state[f"{prefix}_rules"] = snapshot["rules"]
                st.session_state[f"{prefix}_rules_logic"] = snapshot["logic"]
                return apply_rules(prefix, df, tests)
            finally:
                if live_pat is not None: st.session_state[f"{prefix}_pat_rules"] = live_pat
                if live_rules is not None: st.session_state[f"{prefix}_rules"] = live_rules
                if live_logic is not None: st.session_state[f"{prefix}_rules_logic"] = live_logic

        dA = apply_with_snapshot("A", dA_q, A)
        dB = apply_with_snapshot("B", dB_q, B)

        if dA.empty or dB.empty:
            st.warning("One of the cohorts is empty after filters/rules."); return

        mA = compute_km_dataset(dA, tests, cfg)
        mB = compute_km_dataset(dB, tests, cfg)
        ...
        with st.expander("📊 Statistics"):
            try:
                res = logrank_test(mA["km_time"], mB["km_time"],
                                   event_observed_A=mA["km_event"], event_observed_B=mB["km_event"])
            ...
            try:
                ...
                ci_low, ci_high = cph.confidence_intervals_.loc["group"].values
        ...
        with st.expander("📥 Export"):
            ...
            cols = ["__cohort","patient_id","km_time","km_event","km_time_label",
                    "diagnosis_date","surgery_date","first_treatment_date","progression_date",
                    "death_date","last_followup_date","next_treatment_date","cancer_type","stage_group"]
            csv = out[[c for c in cols if c in out.columns]].to_csv(index=False).encode("utf-8")
            st.download_button("Download KM analysis dataset (CSV)", data=csv,
                               file_name="km_onc_analysis.csv", mime="text/csv")

def main():
    render_css(minimal=st.session_state.get("minimal_ui", False))
    tabs = make_tabs()
    with tabs[0]:
        st.markdown('<div class="header-bar"><div class="header-title">QS Oncology Stats (POC v5.1.4)</div><div class="header-sub">Presets for A/B • Patient-level toggles • Rule builder</div></div>', unsafe_allow_html=True)
    data_tab(tabs[0])
    cohorts_tab(tabs[1])
    endpoint_tab(tabs[2])
    km_stats_tab(tabs[3])
