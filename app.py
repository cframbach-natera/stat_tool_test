...
def render_css():
    ...
    :root { --qs-blue:#60A4BF; --qs-blue500:#329ADB; --qs-blue900:#1B5F7F; --qs-green:#7EB86E;
            ...
            --qs-top-offset: 104px; }
    .block-container{padding-top:calc(var(--qs-top-offset) + 12px);padding-bottom:2rem;}
...
/* === Cohort Builder: stronger grayscale overrides & delineation === */

/* Distinct card headers */
...
#cohort-builder .panel-a { background: #FFFFFF !important; border-top: 3px solid var(--qs-gray-700) !important; }
#cohort-builder .panel-b { background: var(--qs-gray-50) !important; border-top: 3px solid var(--qs-gray-700) !important; }

/* Column divider (ensure it renders above backgrounds) */
...
#cohort-builder [data-testid="stHorizontalBlock"]::after {
  ...
  background: var(--qs-gray-300); pointer-events: none;
}

/* FORCE sliders to grayscale in cohort builder */
...
/* Radios — kill any theme colors */
#cohort-builder .stRadio [role="radio"] {
  border: 2px solid var(--qs-gray-500) !important;
  background: #fff !important;
  box-shadow: none !important;
}
#cohort-builder .stRadio [role="radio"][aria-checked="true"] {
  background: var(--qs-ink) !important;
  border-color: var(--qs-ink) !important;
  box-shadow: 0 0 0 2px rgba(16,24,40,.15) !important;
}

/* Multiselect tags */
#cohort-builder [data-baseweb="tag"] {
  background: var(--qs-gray-100) !important;
  color: var(--qs-ink) !important;
  border-color: var(--qs-gray-300) !important;
}
#cohort-builder [data-baseweb="tag"] svg { color: var(--qs-gray-700) !important; }

    
/* === GLOBAL grayscale for cohort builder controls === */
.stSlider [data-baseweb="slider"]>div>div,
.stSlider [data-baseweb="slider"]>div>div>div,
.stSlider [data-baseweb="slider"]>div>div>div>div { background: var(--qs-gray-300)!important; }
.stSlider [role="slider"] { background: var(--qs-gray-700)!important; border-color: var(--qs-gray-700)!important; }
.stSlider [data-testid="stTickBar"] div { background: var(--qs-gray-300)!important; }

.stRadio [role="radio"] { border: 2px solid var(--qs-gray-500)!important; background:#fff!important; box-shadow:none!important; }
.stRadio [role="radio"][aria-checked="true"] { background: var(--qs-ink)!important; border-color: var(--qs-ink)!important; box-shadow: 0 0 0 2px rgba(16,24,40,.15)!important; }

/* Multiselect tags */
[data-baseweb="tag"] { background: var(--qs-gray-100)!important; color: var(--qs-ink)!important; border-color: var(--qs-gray-300)!important; }
[data-baseweb="tag"] svg { color: var(--qs-gray-700)!important; }

    
/* === Tweaks per feedback === */
/* Make slider value labels/text black */
.stSlider * { color: var(--qs-ink) !important; }
.stSlider [data-testid="stThumbValue"] { color: var(--qs-ink) !important; }
/* Selected radio = Natera blue */
.stRadio [role="radio"][aria-checked="true"] {
  background: var(--qs-blue500) !important;
  border-color: var(--qs-blue500) !important;
  box-shadow: 0 0 0 2px rgba(50,154,219,.25) !important;
}

    
/* === Cohort tweaks: keep only top slider labels; radios in blue === */
/* Hide the extra value row that BaseWeb/Streamlit renders *below* the slider */
.stSlider [data-baseweb="slider"] + div { display: none !important; }

/* Ensure the top thumb values stay visible and black */
.stSlider [data-testid="stThumbValue"] { color: var(--qs-ink) !important; }
.stSlider [data-testid="stThumbValue"] * { color: var(--qs-ink) !important; }

/* Radios: use Natera blue for the selected state */
.stRadio input[type="radio"] { accent-color: var(--qs-blue500) !important; }
.stRadio [role="radio"][aria-checked="true"] {
  background: var(--qs-blue500) !important;
  border-color: var(--qs-blue500) !important;
  box-shadow: 0 0 0 2px rgba(50,154,219,.25) !important;
...
/* Radios: force blue when selected (high specificity) */
.stRadio div[role="radiogroup"] label div[role="radio"][aria-checked="true"] {
  background: var(--qs-blue500) !important;
  border-color: var(--qs-blue500) !important;
  box-shadow: 0 0 0 2px rgba(50,154,219,.25) !important;
}
.stRadio div[role="radiogroup"] label div[role="radio"] { border: 2px solid var(--qs-gray-500) !important; background:#fff !important; }
.stRadio input[type="radio"] { accent-color: var(--qs-blue500) !important; }
...
def km_stats_tab(tab):
    with tab:
        ...
        if patients is None or tests is None:
            st.info("Load data in the **Data** tab first."); return
        # Cohorts present & built
        A = st.session_state.get("cohortA"); B = st.session_state.get("cohortB")
        if not A or not B or not (A.get("built") and B.get("built")):
        ...
        # --- Build masks ---
        def mask_from_rules(pat_df, rules, logic):
            if not rules:
                return pd.Series(True, index=pat_df.index)
            masks = []
            for cond in rules:
                try:
                    masks.append(eval_condition(pat_df, tests, cond).reindex(pat_df.index).fillna(False))
                except Exception as e:
                    st.warning(f"Rule error ({cond.get('type','?')}): {e}")
                    masks.append(pd.Series(False, index=pat_df.index))
            if logic == "ANY":
                return pd.concat(masks, axis=1).any(axis=1)
            return pd.concat(masks, axis=1).all(axis=1)

        # Apply quick filters first, then rules
        dA_q = apply_quick_filters(patients, A.get("qf", {}))
        dB_q = apply_quick_filters(patients, B.get("qf", {}))
        mA = mask_from_rules(dA_q, A.get("rules", []), A.get("logic", "ALL"))
        mB = mask_from_rules(dB_q, B.get("rules", []), B.get("logic", "ALL"))
        subA = dA_q.loc[mA].copy()
        subB = dB_q.loc[mB].copy()

        if subA.empty or subB.empty:
            st.info(f"Need >0 patients in each cohort. A={len(subA)}, B={len(subB)}."); return

        # --- Build KM-ready datasets ---
        kmA = compute_km_dataset(subA, tests, cfg)
        kmB = compute_km_dataset(subB, tests, cfg)

        if kmA.empty or kmB.empty:
            st.info("No valid times/events to plot for at least one cohort after filtering."); return

        # --- Plot KM curves ---
        fig, ax = plt.subplots(figsize=(8.5, 5))
        kfA = KaplanMeierFitter(label=A.get("label","Cohort A"))
        kfB = KaplanMeierFitter(label=B.get("label","Cohort B"))
        kfA.fit(kmA["km_time"], event_observed=kmA["km_event"])
        kfB.fit(kmB["km_time"], event_observed=kmB["km_event"])
        kfA.plot(ax=ax, ci_show=True)
        kfB.plot(ax=ax, ci_show=True)
        ax.set_xlabel(kmA["km_time_label"].iloc[0])
        ax.set_ylabel("Survival probability")
        ax.grid(True, axis="y", alpha=0.25)

        # At-risk table
        try:
            add_at_risk_counts(kfA, kfB, ax=ax)
        ...
        with st.expander("📊 Statistics"):
            try:
                res = logrank_test(kmA["km_time"], kmB["km_time"], event_observed_A=kmA["km_event"], event_observed_B=kmB["km_event"])
                st.write(f"Log-rank test: p = **{res.p_value:.4g}**")
            except Exception as e:
                st.info(f"Log-rank failed: {e}")
            try:
                tmp = pd.concat([kmA.assign(group=0), kmB.assign(group=1)], ignore_index=True)
                cph = CoxPHFitter()
